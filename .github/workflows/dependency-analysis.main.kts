...
workflow(
  ...
  job("analyse", runsOn = RunnerType.UbuntuLatest) {
    uses(CheckoutV3())
    uses(GradleBuildActionV2(arguments = "buildHealth"))
    uses(UploadArtifactV3(
      name = "dependency-analysis-output",
      path = listOf("build/reports/dependency-analysis/build-health-report.txt")
    ))
  }
}.writeToFile()